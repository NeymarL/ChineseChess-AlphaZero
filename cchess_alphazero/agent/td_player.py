...
class CChessPlayer:
    ...
    def update_tree(self, p, v, history):
        ...
        if p is not None:
            with self.node_lock[state]:
                ...
                for hist in node.visit:
                    self.executor.submit(self.TD_search, state, hist)
                node.visit = []
        ...
        if len(history) > 0:
            ...
            with self.node_lock[state]:
                ...
                action_state.n += 1 - virtual_loss
                action_state.q = next_state_value
                next_state_value = -node.v
                # logger.debug(f"update value: state = {state}, action = {action}, n = {action_state.n}, q = {action_state.q}")
    ...
    def apply_temperature(self, policy, turn) -> np.ndarray:
        if turn < 20 and self.play_config.tau_decay_rate != 0:
